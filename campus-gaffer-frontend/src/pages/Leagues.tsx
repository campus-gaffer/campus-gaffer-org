...
import { Bell, ChevronRight, Settings, Search, Trophy, Users, Loader2 } from "lucide-react";
...
export default function Leagues() {
  ...
  let campusEntries = allEntries.filter(e =>
    e.university === userUniversity || e.clerk_id === user?.id
  );
  // Deduplicate
  campusEntries = campusEntries.filter((e, i, a) => a.findIndex(x => x.clerk_id === e.clerk_id) === i);
  // Sort by rank
  const campusRanks = campusEntries
    .sort((a, b) => b.total_points - a.total_points)
    .map((e, i) => ({ ...e, rank: i + 1, isUser: e.clerk_id === user?.id }));
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <section className="space-y-6">
              <div className="flex items-center justify-between px-1">
                <h3 className="text-xl md:text-2xl italic font-black text-primary uppercase tracking-widest">
                  {allEntries.length > 3 ? "Global Top 3" : "All Managers"}
                </h3>
                <Trophy className="w-6 h-6 text-primary" />
              </div>

              <div className="space-y-4">
                {(allEntries.length > 3 ? top3 : allEntries).map((item) => {
                  const rank = allEntries.indexOf(item) + 1;
                  return (
                    <div key={item.clerk_id} className="bg-card border border-white/5 rounded-[2.5rem] p-8 flex items-center justify-between hover:border-primary/30 transition-all cursor-pointer group text-left relative overflow-hidden shadow-2xl">
            ...
            <section className="space-y-6">
              <div className="flex items-center justify-between px-1 text-left">
                <h3 className="text-xl md:text-2xl italic font-black text-primary uppercase tracking-widest">
                  {userUniversity ? `${userUniversity.toUpperCase()} STANDINGS` : "Campus Standings"}
                </h3>
                <Users className="w-6 h-6 text-primary" />
              </div>

              <div className="space-y-4">
                {campusRanks.length === 0 ? (
                  <div className="bg-card border border-white/5 rounded-[2.5rem] p-12 text-center">
                    <p className="text-slate-500 font-black italic uppercase tracking-widest text-xs">No campus data yet</p>
                  </div>
                ) : (
                  campusRanks.map((item) => (
                    <div key={item.clerk_id} className={`bg-card border rounded-[2.5rem] p-8 flex items-center justify-between hover:border-primary/30 transition-all cursor-pointer group text-left relative overflow-hidden shadow-2xl ${item.isUser ? 'border-primary ring-2 ring-primary/10 bg-primary/5' : 'border-white/5'}`}>
                      <div className="flex items-center gap-6 relative z-10">
                        <div className="relative">
                        ...
                        <div>
                          <p className={`text-lg md:text-xl font-black uppercase leading-none mb-2 ${item.isUser ? 'text-primary' : 'text-white'}`}>{item.team_name} {item.isUser && "(YOU)"}</p>
                          <p className="text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-widest leading-none">{item.username}</p>
                        </div>
                      </div>
                      ...
                      <div className="text-right flex items-center gap-6 relative z-10">
                        <div>
                          <p className={`text-3xl md:text-4xl font-black italic leading-none ${item.isUser ? 'text-white' : 'text-primary'}`}>{item.total_points}</p>
                          <p className="text-[10px] font-bold text-slate-600 uppercase text-right mt-1 tracking-widest">PTS</p>
                        </div>
                        <ChevronRight className="w-6 h-6 text-slate-700 group-hover:text-primary transition-colors" />
                      </div>
